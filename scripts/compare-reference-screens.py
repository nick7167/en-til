"""Create aspect-preserving review sheets; --stretch reproduces old normalized comparisons."""
from pathlib import Path
import json,sys
from PIL import Image,ImageDraw,ImageOps
ROOT=Path(__file__).resolve().parents[1]
folder=Path(sys.argv[1]);shots=folder/'screenshots';out=folder/'comparison';out.mkdir(exist_ok=True)
# Preserve phone proportions so review does not mistake stretching for a layout defect.
def thumbnail(image):
 if '--stretch' in sys.argv[2:]: return image.resize((174,394),Image.Resampling.LANCZOS)
 return ImageOps.pad(image,(174,394),method=Image.Resampling.LANCZOS,color='#ededeb')
...
for letter,file,boxes in refs:
 ...
 for i,box in enumerate(boxes):
  ...
  original=thumbnail(sheet.crop(box))
  ...
  if found:
   actual=thumbnail(Image.open(shots/found['exportedFileName']).convert('RGB'))
