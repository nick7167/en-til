...
StoreKit checkpoint correction: the configuration now belongs to the app as a resource, excluded from Release builds. Only DEBUG simulator screenshot launches initialize and retain `SKTestSession` inside the app, and preview views wait until that setup has loaded products. UI tests assert prices without owning another StoreKit session. This replaces the unsuccessful test-runner-only configuration; hosted verification remains required.
